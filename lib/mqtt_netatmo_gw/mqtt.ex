defmodule MqttNetatmoGw.Mqtt do
  alias MqttNetatmoGw.Mqtt.Handler

  @spec client :: <<_::64, _::_*8>>
  def client(), do: "netatmo_gw_#{Enum.random(1..9)}"
  ...
  def connect(client_id) do
    config = Application.get_env(:mqtt_netatmo_gw, :mqtt)
    ...
    case Tortoise.Supervisor.start_child(
      ...
      subscriptions: [{"#{config[:cmd_topic_namespace]}/weatherstation/+", 0}],
  ...
  def publish(client_id, key, value) when is_binary(client_id) and is_binary(key) do
    topic_ns = Application.get_env(:mqtt_netatmo_gw, :mqtt)[:event_topic_namespace]
    topic = sanitize_topic("#{topic_ns}/weatherstation/#{String.downcase(key)}")
